...
public class Health : MonoBehaviour
{
    ...
    [Tooltip("The Maximum Armor Value")]
    public int MaxArmor = 1;
    [Tooltip("The maximum health value")]
    public int MaxHealth = 1;
    [Tooltip("in seconds, after taking damage")]
    public float TimeBeforeRecharge = 3f;
    ...
    private bool startRecharge = false;
    ...
    public void TakeDamage(float damageAmount)
    {
        if (isEnemy)
        {
            ...
            if (isBoss)
            {
                damageeee += damageAmount;
                Broker.BossDamage(damageAmount);
                CheckedHp();
            }
            CheckDeath();
            return;
